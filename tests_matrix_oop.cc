...
TEST(Set, True) {
  ...
  result(2, 0) = 5;
  result(2, 1) = -2;
  matrix_a.Resize(matrix_a.GetRows(), 2);
  ...
  matrix_b.Resize(2, matrix_b.GetCols());
  ...
  ASSERT_ANY_THROW(matrix_c.Resize(-5, matrix_c.GetCols()));

  ASSERT_ANY_THROW(matrix_c.Resize(matrix_c.GetRows(), 0));
}
